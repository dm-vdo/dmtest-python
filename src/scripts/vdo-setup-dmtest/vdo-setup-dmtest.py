...
class vdoDmtest(object):
  ...
  # Dictionary of valid device keys and boolean indicating whether they are required
  deviceConfigKeys = {'DATA_DEVICE' : True, 'METADATA_DEVICE' : False}
  ...
  def verifyDevices(self):
    """
    Verify that the expected device configuration parameters exist and correspond to block devices
    """
    self.log.info("Verifying the device(s) defined in the configuration file")
    for key, isRequired in self.deviceConfigKeys.items():
      self.log.debug("Searching config for device key '{0}'".format(key))
      try:
        # Verify required device key(s) exist and are defined
        if isRequired:
          prefixStr = "Required device key '{0}'".format(key)
          assert key in self.config.keys(), "{0} not found".format(prefixStr)
          assert self.config[key], "{0} is undefined".format(prefixStr)
        # Verify optional device key(s), if present
        else:
          prefixStr = "Optional device key '{0}'".format(key)
          if key in self.config.keys():
            self.log.debug("{0} found".format(prefixStr))
            if self.config[key] is not None:
              self.log.debug("{0} is defined".format(prefixStr))
            else:
              self.log.debug("{0} is undefined".format(prefixStr))
              continue
          else:
            self.log.debug("{0} not found".format(prefixStr))
            continue

        # Verify the specified device is a block device
        device = self.config[key]
        assert os.path.exists(device), "{prefix} {msg}".format(prefix = prefixStr,
          msg = 'is defined with a path that does not exist')

        mode = os.stat(device).st_mode
        assert stat.S_ISBLK(mode), "{prefix} {msg}".format(prefix = prefixStr,
          msg = 'does not correspond to a valid block device')
      except Exception as error:
        self.log.error("{err}".format(err = error))
        sys.exit(1)
  ...
  def createDmtestConfig(self):
    ...
    try:
      # Whether the devices are required or optional is handled when initially processing the
      # configuration file. As such, search for the device keys and associate their values with
      # the corresponding variables if they are not undefined.
      data_device = ''
      if 'DATA_DEVICE' in self.config and self.config['DATA_DEVICE'] is not None:
        data_device = self.config['DATA_DEVICE']

      metadata_device = ''
      if 'METADATA_DEVICE' in self.config and self.config['METADATA_DEVICE'] is not None:
        metadata_device = self.config['METADATA_DEVICE']

      # Write the devices and necessary parameters
      with open(configFile, 'w+') as fh:
        fh.write("metadata_dev = '%s'\ndata_dev = '%s'\n%s\n" % (metadata_device, data_device,
